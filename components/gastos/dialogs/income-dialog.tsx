...
import { INCOME_CATEGORY_OPTIONS } from "@/lib/gastos/constants";
    ...
    <form onSubmit={handleSubmit} className="space-y-4 mt-2">
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="Ej: Zumba mayo, Clases de spinning"
          required
          className={inputCls}
        />
      </div>
...
export function IncomeDialog({ open, onClose }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="border-zinc-800 bg-zinc-900 text-white max-w-md">
        <DialogHeader>
          <DialogTitle className="text-zinc-100">Registrar ingreso</DialogTitle>
        </DialogHeader>
        <IncomeForm key={formKey} onClose={onClose} />
      </DialogContent>
    </Dialog>
  );
}
